...
class SophiaModelEvaluacionesutp extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->filter_state == 'P') {
			$where[] = 'eva.published = 1';
		}
		elseif ($this->filter_state == 'U') {
			$where[] = 'eva.published = 0';
		}

		if ($this->search)
		{
			$where[] = 'LOWER(eva.titulo) LIKE \'%'. $this->search. '%\'';
		}
		...
		$this->_query = ' SELECT eva.*,cur.curso AS curso , prof.nombre AS profesor,asig.asignatura as asignatura,per.nombre as periodo'
		. ' FROM #__sophia_evaluacion AS eva'
