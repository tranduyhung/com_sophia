...
<form action="index.php" method="post" name="adminForm">
		...
		<table class="adminlist">
			<thead>
				<tr>
					...
					<th width="30%" class="title"><?php echo JHTML::_('grid.sort',   'asignatura', 'asignatura', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
			...
			<?php
			$k = 0;
			for ($i=0, $n=count( $this->materias ); $i < $n; $i++)
			{
				$row = &$this->materias[$i];

				$link 		= JRoute::_( 'index.php?option=com_sophia&controller=materia&task=edit&cid[]='. $row->id);
				$linkpublicar		= JRoute::_( 'index.php?option=com_sophia&controller=materia&task=publish&cid[]='. $row->id);
				$linkdespublicar	= JRoute::_( 'index.php?option=com_sophia&controller=materia&task=unpublish&cid[]='. $row->id);
	...
	<input type="hidden" name="option" value="com_sophia" /> <input
		...
		type="hidden" name="controller" value="materia" /> <input
		type="hidden" name="view" value="materias" />
