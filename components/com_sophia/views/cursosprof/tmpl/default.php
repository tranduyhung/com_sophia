...
echo SophiaHelperToolbar::IconButtonEdit( 'cursoprof', 'icons/nuevo.png', JText::_( 'Nuevo' ),JText::_( 'Nuevo' ),JText::_( 'Descripcion' ) );
...
<form action="index.php" method="post" name="adminForm">
		...
		<table class="adminlist">
			<thead>
				<tr>
					...
					<th width="30%" class="title"><?php echo JHTML::_('grid.sort',   'profesor', 'profesor', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
			...
			<?php
			$k = 0;
			for ($i=0, $n=count( $this->cursosprof ); $i < $n; $i++)
			{
				$row = &$this->cursosprof[$i];

				$link 		= JRoute::_( 'index.php?option=com_sophia&controller=cursoprof&task=edit&cid[]='. $row->id);
				$linkpublicar		= JRoute::_( 'index.php?option=com_sophia&controller=cursoprof&task=publish&cid[]='. $row->id);
				$linkdespublicar	= JRoute::_( 'index.php?option=com_sophia&controller=cursoprof&task=unpublish&cid[]='. $row->id);
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td><span class="editlinktip hasTip"
						title="<?php echo JText::_( 'EDIT' );?>::<?php echo JText::_( 'EDIT_DESC' );?>">
							<a href="<?php echo $link  ?>"> <?php echo $row->curso; ?> </a> </span>
					</td>
	...
	<input type="hidden" name="option" value="com_sophia" /> <input
		...
		type="hidden" name="controller" value="cursoprof" /> <input
		type="hidden" name="view" value="cursosprof" />
