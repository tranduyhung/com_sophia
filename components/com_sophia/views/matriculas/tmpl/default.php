...
echo SophiaHelperToolbar::IconButtonEdit( 'matricula', 'icons/nuevo.png', JText::_( 'Nuevo' ),JText::_( 'Nuevo' ),JText::_( 'descripcion Nuevo Alumno' ) );
...
<form action="index.php" method="post" name="adminForm">
		...
		<table class="adminlist">
			<thead>
				<tr>
					...
					<th width="10%" class="title"><?php echo JHTML::_('grid.sort',   'nromatricula', 'nromatricula', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
					<th width="5%" class="title"><?php echo JHTML::_('grid.sort',   'anno', 'anno', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
					<th width="30%" class="title"><?php echo JHTML::_('grid.sort',   'curso', 'curso', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
			...
			<?php
			$k = 0;
			for ($i=0, $n=count( $this->matriculas ); $i < $n; $i++)
			{
				$row = &$this->matriculas[$i];

				$link 		= JRoute::_( 'index.php?option=com_sophia&controller=matricula&task=edit&cid[]='. $row->id);
				$linkpublicar		= JRoute::_( 'index.php?option=com_sophia&controller=matricula&task=publish&cid[]='. $row->id);
				$linkdespublicar	= JRoute::_( 'index.php?option=com_sophia&controller=matricula&task=unpublish&cid[]='. $row->id);
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center"><span class="editlinktip hasTip"
						title="<?php echo JText::_( 'EDIT' );?>::<?php echo JText::_( 'EDIT_DESC' );?>">
							<a href="<?php echo $link  ?>"> <?php echo $row->nromatricula; ?>
						</a> </span>
					</td>
	...
	<input type="hidden" name="option" value="com_sophia" /> <input
		...
		type="hidden" name="controller" value="matricula" /> <input
		type="hidden" name="view" value="matriculas" />
