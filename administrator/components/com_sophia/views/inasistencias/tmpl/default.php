...
<form action="index.php" method="post" name="adminForm">
	...
	<div id="tablecell">
		<table class="adminlist">
			<thead>
				<tr>
					...
					<th width="2%"><input type="checkbox" name="toggle" value=""
						onclick="checkAll(<?php echo count( $this->anotacion ); ?>);" />
					</th>
					<th width="30%" class="title"><?php echo JHTML::_('grid.sort',   'fecha', 'fecha', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
					<th width="5%" class="title"><?php echo JHTML::_('grid.sort',   'periodo', 'periodo', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
					<th width="10%" class="title"><?php echo JHTML::_('grid.sort',   'curso', 'curso', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
					<th width="20%" class="title"><?php echo JHTML::_('grid.sort',   'alumno', 'alumno', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
			...
			for ($i=0, $n=count( $this->inasistencias ); $i < $n; $i++)
			{
				...
				$link 		= JRoute::_( 'index.php?option=com_sophia&controller=inasistencia&task=edit&cid[]='. $row->id);
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td align="center"><span class="editlinktip hasTip"
						title="<?php echo JText::_( 'Edit Item' );?>::<?php echo JText::_( 'Edit Item DESC' );?>">
							<a href="<?php echo $link  ?>"> <?php echo $row->fecha; ?> </a> </span>
					</td>
	...
	<input type="hidden" name="option" value="com_sophia" /> <input
		...
		type="hidden" name="controller" value="inasistencia" /> <input
		type="hidden" name="view" value="inasistencias" />
