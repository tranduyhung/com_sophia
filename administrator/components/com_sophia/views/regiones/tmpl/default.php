...
<form action="index.php" method="post" name="adminForm">
		...
		<table class="adminlist">
			<thead>
				<tr>
					...
					<th width="50%" class="title"><?php echo JHTML::_('grid.sort',   'Region', 'Region', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
			...
			<?php
			$k = 0;
			for ($i=0, $n=count( $this->regiones ); $i < $n; $i++)
			{
				$row = &$this->regiones[$i];

				$link 		= JRoute::_( 'index.php?option=com_sophia&controller=region&task=edit&cid[]='. $row->id);
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td><span class="editlinktip hasTip"
						title="<?php echo JText::_( 'Edit Item' );?>::<?php echo JText::_( 'Edit Item DESC' );?>">
							<a href="<?php echo $link  ?>"> <?php echo $row->region; ?> </a>
					</span>
					</td>
	...
	<input type="hidden" name="option" value="com_sophia" /> <input
		...
		type="hidden" name="controller" value="region" /> <input type="hidden"
		name="view" value="regiones" />
