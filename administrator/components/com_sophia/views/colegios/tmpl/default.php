...
<form action="index.php" method="post" name="adminForm">
	...
	<div id="tablecell">
		<table class="adminlist">
			<thead>
				<tr>
					...
					<th width="5%"><input type="checkbox" name="toggle" value=""
						onclick="checkAll(<?php echo count( $this->colegios ); ?>);" />
					</th>
					<th width="10%" class="title"><?php echo JHTML::_('grid.sort',   'Establecimiento', 'Establecimiento', @$this->lists['order_Dir'], @$this->lists['order'] ); ?>
					</th>
					<th width="10%" class="title"><?php echo JText::_( 'Direccion' ); ?>
					</th>
					<th width="10%" class="title"><?php echo JText::_( 'Telefono' ); ?>
					</th>
					<th width="20%" class="title"><?php echo JText::_( 'Director' ); ?>
					</th>
			...
			<?php
			$k = 0;
			for ($i=0, $n=count( $this->colegios ); $i < $n; $i++)
			{
				$row = &$this->colegios[$i];

				$link 		= JRoute::_( 'index.php?option=com_sophia&controller=colegio&task=edit&cid[]='. $row->id);
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td><span class="editlinktip hasTip"
						title="<?php echo JText::_( 'Edit Item' );?>::<?php echo JText::_( 'Edit Item DESC' );?>">
							<a href="<?php echo $link  ?>"> <?php echo $row->establecimiento; ?>
						</a> </span>
					</td>
	...
	<input type="hidden" name="option" value="com_sophia" /> <input
		...
		type="hidden" name="controller" value="colegio" /> <input
		type="hidden" name="view" value="colegios" />
