...
class SophiaModelMaterias extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->filter_state == 'P') {
			$where[] = 'al.published = 1';
		}
		elseif ($this->filter_state == 'U') {
			$where[] = 'al.published = 0';
		}
		...
		$where[] = 'al.profesor_id = prof.id';
		$where[] = 'al.curso_id = cur.id';
		$where[] = 'al.asignatura_id = asig.id';

		$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : '' );

		$orderby = '';
		if (($this->filter_order != 'curso') && ($this->filter_order != 'profesor')&& ($this->filter_order != 'asignatura') && ($this->filter_order != 'published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT al.id , al.published ,cur.curso AS curso , prof.nombre AS profesor,asig.asignatura '
		. ' FROM #__sophia_materias AS al,#__sophia_curso AS cur, #__sophia_profesores AS prof,#__sophia_asignaturas AS asig '
		. $where
		. $orderby
		;
