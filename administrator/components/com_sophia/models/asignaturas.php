...
class SophiaModelAsignaturas extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->search)
		{
			$where[] = 'LOWER(asignatura) LIKE \''. $this->search. '\'';
		}

		$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : '' );

		$orderby = '';
		if (($this->filter_order != 'asignatura')  && ($this->filter_order != 'published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT al.*'
		. ' FROM #__sophia_asignaturas AS al'
		. $where
		. $orderby
		;
		return $this->_query;
