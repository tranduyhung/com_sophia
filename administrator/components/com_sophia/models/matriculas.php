...
class SophiaModelMatriculas extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->filter_state == 'P') {
			$where[] = 'mat.published = 1';
		}
		elseif ($this->filter_state == 'U') {
			$where[] = 'mat.published = 0';
		}
		...
		$where[] = ' mat.alumno_id = est.id';
		$where[] = ' mat.curso_id = cur.id';
		$where[] = ' mat.anno_id = anno.id';


		$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : '' );

		$orderby = '';
		if (($this->filter_order != 'curso') && ($this->filter_order != 'alumno') && ($this->filter_order != 'published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT mat.* ,cur.curso AS curso , est.nombre AS alumno , anno.anno As anno '
		. ' FROM #__sophia_matricula AS mat,#__sophia_curso AS cur, #__sophia_estudiantes AS est , #__sophia_anno AS anno '
		. $where
		. $orderby
		;
