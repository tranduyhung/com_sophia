...
class SophiaModelAranceles extends JModel
{
	...
	function _buildQuery()
	{
		...
		if ($this->filter_state == 'P') {
			$where[] = 'ara.published = 1';
		}
		elseif ($this->filter_state == 'U') {
			$where[] = 'ara.published = 0';
		}
		...
		$where[] = ' ara.matricula_id = mat.id';
		$where[] = ' mat.alumno_id = es.id';
		$where[] = ' ara.anno_id = an.id';

		$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : '' );


		$orderby = '';
		if (($this->filter_order != 'es.alumno')  && ($this->filter_order != 'ara.published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT ara.*,es.nombre as alumno,an.anno as anno'
