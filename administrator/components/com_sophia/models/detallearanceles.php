...
class SophiaModelaranceles extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->search)
		{
			$where[] = 'LOWER(es.nombre) LIKE \''. $this->search. '\'';
		}

		$where[] = ' al.matricula_id = mat.id';
		$where[] = ' mat.alumno_id = es.id';
		$where[] = ' al.anno_id = an.id';
		$where[] = ' al.id = det.arancel_id';

		$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : '' );


		$orderby = '';
		if (($this->filter_order != 'alumno')  && ($this->filter_order != 'published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT det.*,es.nombre as alumno,an.anno as anno'
		. ' FROM #__sophia_arancel AS al,#__sophia_estudiantes as es ,#__sophia_matriculas as mat ,#__sophia_anno as an,#__sophia_ara_detalle As det'
		. $where
		. $orderby
		;
		return $this->_query;
