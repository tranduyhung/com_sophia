...
class SophiaModelColegios extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->search)
		{
			$where[] = 'LOWER(establecimiento) LIKE \''. $this->search. '\'';
		}

		$where 		= ( count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : '' );

		$orderby = '';
		if (($this->filter_order != 'published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT *'
		. ' FROM #__sophia_colegio '
		. $where
		. $orderby
		;
		return $this->_query;
