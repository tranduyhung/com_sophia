...
class SophiaModelperiodos extends JModel
{
	var $_data;
	...
	function _buildQuery()
	{
		...
		if ($this->filter_state == 'P') {
			$where[] = 'anno.published = 1';
		}
		elseif ($this->filter_state == 'U') {
			$where[] = 'anno.published = 0';
		}

		$where[] = 'anno.id = al.anno_id';
		...
		$orderby = '';
		if (($this->filter_order != 'nombre')  && ($this->filter_order != 'anno')  && ($this->filter_order != 'published') && ($this->filter_order != 'id')) $this->filter_order = '';
		if (($this->filter_order) && ($this->filter_order_Dir))
		$orderby 	= ' ORDER BY '. $this->filter_order .' '. $this->filter_order_Dir;

		$this->_query = ' SELECT al.*,anno.anno As anno'
		. ' FROM #__sophia_periodo AS al,#__sophia_anno As anno'
		. $where
		. $orderby
		;
		return $this->_query;
